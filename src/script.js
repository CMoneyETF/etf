...
const ETFDividendTool = () => {
  ...
  return (
    <div className="p-4 max-w-[1200px] mx-auto">
      <h1 className="text-2xl font-bold mb-6">2024 ETF 配息年表查詢工具</h1>
        ...
        <table className="w-full bg-white shadow-md rounded">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-4 py-2">配息週期</th>
              <th className="px-4 py-2">今年配息</th>
              <th className="px-4 py-2">相較去年</th>
              <th className="px-4 py-2">年化殖利率</th>
          ...
          <tbody>
            {filteredETFs.map(etf => (
              <tr key={etf.code} className="border-b">
                <td className="px-4 py-2">{etf.code}</td>
                <td className="px-4 py-2">{etf.name}</td>
                <td className="px-4 py-2">{etf.period}</td>
                <td className="px-4 py-2">{etf.dividend}</td>
                <td className={`px-4 py-2 ${getComparedColor(etf.compared)}`}>{etf.compared}</td>
                <td className="px-4 py-2">{etf.yield}</td>
                <td className="px-4 py-2">{etf.category}</td>
              </tr>
            ))}
          </tbody>
        </table>
